...
os.makedirs("saved_checkpoints_3", exist_ok=True)
...
for epoch in range(num_epochs):
    ...
    if val_eer < best_val_eer:
        ...
        with open("best_threshold_3.txt", "w") as f:
            f.write(str(best_threshold_3))
        print(f"New best threshold saved: {best_threshold_3:.4f} (EER: {val_eer:.4f})")

        torch.save(model.state_dict(), f"saved_checkpoints_3/epoch_{epoch+1}_EER_{val_eer:.4f}.pth")
...
torch.save(swa_model.module.state_dict(), "swa_rawnet2_3.pth")
print("Saved SWA model to swa_rawnet3.pth")
