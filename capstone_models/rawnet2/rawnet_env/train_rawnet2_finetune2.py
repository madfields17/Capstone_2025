...
class DeepfakeAudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = 0 if row['spoof_or_real'] == "spoof" else 1
...
best_threshold = None
os.makedirs("saved_checkpoints_2", exist_ok=True)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for batch_x, batch_y in tqdm(val_loader, desc=f"[Epoch {epoch+1}] Validation"):
            outputs = model(batch_x)
            loss = criterion(outputs, batch_y)
            probs = torch.softmax(outputs, dim=1)[:, 1]  # bonafide prob
    ...
    if val_eer < best_val_eer:
        ...
        with open("best_threshold_2.txt", "w") as f:
            f.write(str(best_threshold_2))
        print(f"New best threshold saved: {best_threshold_2:.4f} (EER: {val_eer:.4f})")

        torch.save(model.state_dict(), f"saved_checkpoints_2/epoch_{epoch+1}_EER_{val_eer:.4f}.pth")
...
torch.save(swa_model.module.state_dict(), "swa_rawnet2_2.pth")
